...
def process_symbol(sym, prefix, year, date=None):
    ...
    for dt, row in sorted(fut_dates.items()):
        ...
        nearest_expiry = None
        nearest_code = None  # 最近到期期权对应的标的期货合约号（如 m2201）
        nearest_dte = 0
        seen_codes = set()
        ...
        pt_vol = opt.pivot_table(index='strike', columns='type', values='opt_volume', aggfunc='sum')
        pt_close = opt.pivot_table(index='strike', columns='type', values='opt_close', aggfunc='mean')
        chain_list = []
